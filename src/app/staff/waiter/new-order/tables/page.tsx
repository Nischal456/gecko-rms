...
function TableCard({ table, onClick }: any) {
    ...
    return (
        <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            onClick={onClick}
            className={`relative flex flex-col p-4 md:p-5 rounded-2xl md:rounded-[1.25rem] border-2 transition-all duration-300 text-left group w-full h-full min-h-[150px] md:min-h-[160px] justify-between ${borderColor} ${bgColor}`}
        >
            {/* Header: Table Number & Status */}
            <div className="flex justify-between items-center w-full gap-2 min-w-0">
                <div className={`h-9 md:h-10 min-w-[2.75rem] md:min-w-[3.25rem] px-2.5 rounded-xl flex-shrink-0 flex items-center justify-center border shadow-sm transition-colors ${isOccupied ? 'bg-white border-orange-100' : 'bg-emerald-50 border-emerald-100'}`}>
