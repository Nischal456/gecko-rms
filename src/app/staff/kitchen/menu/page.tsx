...
import { 
  ...
  History, LogOut, RefreshCcw, Volume2, VolumeX, FileBarChart
} from "lucide-react";
...
function KitchenDock() {
    ...
    return (
        <motion.div 
            ...
            className="fixed bottom-8 left-[50%] z-50 flex items-center gap-3 p-2.5 bg-slate-900/90 backdrop-blur-3xl rounded-full shadow-2xl border border-white/10 ring-1 ring-black/40"
        >
            ...
            <Link href="/staff/kitchen/menu" className="p-3.5 rounded-full bg-emerald-500 text-white shadow-lg shadow-emerald-500/30"><LayoutGrid className="w-5 h-5" /></Link>
            <Link href="/staff/kitchen/reports" className="p-3.5 rounded-full bg-white/5 text-slate-300 hover:text-white"><FileBarChart className="w-5 h-5" /></Link>
            <div className="w-px h-6 bg-white/10 mx-1" />
...
export default function KitchenMenuPage() {
  ...
  async function handleDeleteItem(itemId: string) {
      ...
      await deleteKitchenItem(activeTabId, itemId); // PASS CATEGORY ID
      loadData();
  ...
  async function handleQuickToggle(item: MenuItem) {
      ...
      await quickToggleItem(activeTabId, item.id, !item.is_available); // PASS CATEGORY ID
  }
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative">
        ...
        <header className="flex-shrink-0 px-4 md:px-8 py-4 bg-white/90 backdrop-blur-md border-b border-slate-200 flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 z-20">
            <div className="flex items-center gap-3">
                ...
                <div><h1 className="text-xl font-black text-slate-900 leading-tight">Menu Manager</h1><p className="text-[10px] font-bold text-emerald-600 uppercase tracking-wide">Kitchen Edition</p></div>
            </div>
        ...
        <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
            ...
            {/* GRID */}
            <div className="flex-1 bg-[#F8FAFC] p-4 md:p-8 overflow-y-auto custom-scrollbar pb-32">
                {itemsToDisplay.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center text-slate-400 opacity-50 min-h-[300px]"><LayoutGrid className="w-16 h-16 mb-4" /><p className="font-bold">No dishes in this category.</p></div>
                ) : (
