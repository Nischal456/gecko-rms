...
import { 
  ...
  X, UploadCloud, Database, Save, LayoutGrid,
  ChefHat, Wine, Coffee, Leaf, Beef, GlassWater, Layers, ToggleLeft, ToggleRight,
  History, LogOut, RefreshCcw, Volume2, VolumeX, FileBarChart, Bell, Ticket
} from "lucide-react";
...
export default function KitchenMenuPage() {
  ...
  async function loadData() {
    ...
    // 1. Fetch Tenant settings to check KOT/BOT Split Status
  ...
  const setTab = (id: string) => { setActiveTabId(id); localStorage.setItem("gecko_kitchen_cat", id); }
  ...
  if (loading) return (
      <div className="min-h-screen flex flex-col gap-4 items-center justify-center bg-[#F8FAFC]">
          ...
          <span className="text-xs font-black uppercase tracking-widest text-slate-400 animate-pulse">Syncing Menu...</span>
      </div>
  );
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative pb-[100px] md:pb-0">
        ...
        <header className="flex-shrink-0 px-4 md:px-8 py-4 bg-white/90 backdrop-blur-xl border-b border-slate-200 flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 z-20 shadow-[0_4px_20px_rgb(0,0,0,0.02)]">
            <div className="flex items-center gap-3">
                ...
                <div>
                    <h1 className="text-xl font-black text-slate-900 leading-tight">Menu Manager</h1>
                    <div className="flex items-center gap-2 mt-0.5">
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">Kitchen Edition</p>
                        {isSplitActive && <span className="bg-orange-100 text-orange-700 px-1.5 py-0.5 rounded text-[8px] font-black uppercase tracking-widest flex items-center gap-1 border border-orange-200"><Ticket className="w-2.5 h-2.5" /> KOT Only</span>}
                    </div>
                </div>
            </div>
            ...
            <div className="flex-1 bg-[#F8FAFC] p-4 md:p-8 overflow-y-auto custom-scrollbar">
                {itemsToDisplay.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center text-slate-400 opacity-50 min-h-[300px]">
                        ...
                        <p className="font-black text-lg">No dishes found.</p>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
                    ...
                    <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 50, opacity: 0 }} className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl relative z-10 p-6 md:p-8 border border-slate-100 max-h-[90vh] overflow-y-auto custom-scrollbar">
                        <div className="flex justify-between items-center mb-6">
                            <h2 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight">{editingItem ? "Edit Dish" : "New Dish"}</h2>
                            <button onClick={() => setIsItemModalOpen(false)} className="p-2 bg-slate-50 hover:bg-slate-100 text-slate-500 rounded-full transition-colors"><X className="w-6 h-6" /></button>
                        </div>
                        ...
                        <form onSubmit={handleSaveItem} className="space-y-5">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="col-span-2">
                                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1 mb-1.5 block">Dish Name</label>
                                    <input value={formName} onChange={e => setFormName(e.target.value.replace(/[^a-zA-Z0-9\s]/g, ''))} required placeholder="e.g. Signature Burger" className="w-full h-12 px-4 bg-white border border-slate-200 shadow-sm rounded-xl font-bold text-slate-900 outline-none focus:ring-4 focus:ring-emerald-50 focus:border-emerald-400 transition-all" />
                                </div>
                                ...
                                <div className={variants.length > 0 ? "opacity-50 pointer-events-none" : ""}>
                                    ...
                                    <div className="relative">
                                        ...
                                        <input 
                                            ...
                                            placeholder="0" 
                                            className="w-full h-12 pl-10 pr-4 bg-white border border-slate-200 shadow-sm rounded-xl font-bold text-slate-900 outline-none focus:ring-4 focus:ring-emerald-50 focus:border-emerald-400 transition-all" 
                                        />
                                    </div>
                                </div>

                                <div>
                                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1 mb-1.5 block">Sub-Category</label>
                                    <input value={formSubCat} onChange={e => setFormSubCat(e.target.value.replace(/[^a-zA-Z0-9\s]/g, ''))} placeholder="e.g. Spicy" className="w-full h-12 px-4 bg-white border border-slate-200 shadow-sm rounded-xl font-bold text-slate-900 outline-none focus:ring-4 focus:ring-emerald-50 focus:border-emerald-400 transition-all" />
                                </div>
                            </div>
                            ...
                            <div className="flex gap-4">
                                <div className="flex-1">
                                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1 mb-1.5 block">Dietary Type</label>
                                    <select value={formDietary} onChange={e => setFormDietary(e.target.value)} className="w-full h-12 px-4 bg-white border border-slate-200 shadow-sm rounded-xl font-bold text-slate-700 outline-none focus:ring-2 focus:ring-emerald-100">
                            ...
                            <div className="bg-slate-50 p-5 rounded-[1.5rem] border border-slate-100">
                                <div className="flex justify-between items-center mb-4">
                                    ...
                                    <button type="button" onClick={addVariant} className="text-[10px] font-black uppercase tracking-wider bg-white border border-slate-200 shadow-sm px-3 py-1.5 rounded-lg hover:bg-emerald-50 hover:text-emerald-600 hover:border-emerald-200 transition-colors">+ Add Size/Type</button>
                                </div>
                                <div className="space-y-3">
                                    {variants.map((v, i) => (
                                        <div key={i} className="flex gap-2">
                                            <input value={v.name} onChange={e => updateVariant(i, "name", e.target.value.replace(/[^a-zA-Z0-9\s]/g, ''))} placeholder="Type (e.g. Regular, Large)" className="flex-1 h-11 px-4 rounded-xl border border-slate-200 text-sm font-bold outline-none focus:border-emerald-400" />
                                            <div className="relative w-28">
                                                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs font-bold text-slate-400">Rs</span>
                                                <input type="number" value={v.price || ""} onChange={e => updateVariant(i, "price", Math.max(0, Number(e.target.value)))} placeholder="0" className="w-full h-11 pl-8 pr-3 rounded-xl border border-slate-200 text-sm font-black text-slate-900 outline-none focus:border-emerald-400" />
                                            </div>
                            ...
                            <div className="flex items-center justify-between p-5 bg-white border border-slate-200 shadow-sm rounded-[1.5rem]">
                                <div>
                                    <span className="font-black text-sm text-slate-900 block">Available to Order?</span>
                                    <span className="text-[10px] font-bold text-slate-400">Turn off if out of stock</span>
                                </div>
                                <button type="button" onClick={() => setFormAvailable(!formAvailable)} className={`text-[40px] leading-none transition-colors ${formAvailable ? 'text-emerald-500 hover:text-emerald-600' : 'text-slate-300 hover:text-slate-400'}`}>
                            ...
                            <div>
                                ...
                                <textarea value={formDesc} onChange={e => setFormDesc(e.target.value)} placeholder="Delicious details about this dish..." className="w-full h-24 p-4 bg-white border border-slate-200 shadow-sm rounded-xl font-bold resize-none text-slate-700 outline-none focus:ring-4 focus:ring-emerald-50 focus:border-emerald-400 transition-all" />
                            </div>
...
function ItemCard({ item, onEdit, onDelete, onToggle }: any) {
    ...
    if(isVeg) { BadgeIcon = Leaf; badgeColor = 'bg-emerald-100 text-emerald-700'; }
