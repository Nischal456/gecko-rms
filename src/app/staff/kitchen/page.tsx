        ...
        <header className="flex-shrink-0 bg-white/80 backdrop-blur-xl border-b border-slate-200/60 px-4 md:px-6 py-3 flex justify-between items-center z-30 sticky top-0 shadow-sm transition-all">
            <AnimatePresence>
                {alertingTable && (
                    <motion.div 
                        ...
                        className="absolute inset-0 bg-red-500 flex items-center justify-between px-4 md:px-6 z-50 text-white shadow-xl cursor-pointer"
                    ...
                    >
                        <div className="flex items-center gap-3 md:gap-4 animate-pulse">
                            <div className="w-8 h-8 md:w-10 md:h-10 bg-white/20 rounded-full flex items-center justify-center backdrop-blur-sm">
                                <Bell className="w-4 h-4 md:w-5 md:h-5 fill-current" />
                            </div>
                            <div>
                                <p className="text-[9px] md:text-[10px] font-bold opacity-90 uppercase tracking-widest">New Order Pending</p>
            ...
            <div className="flex items-center gap-3 md:gap-5">
                ...
                <div className="hidden md:block">
                    <h1 className="text-lg md:text-2xl font-black text-slate-900 tracking-tight leading-none">Kitchen<span className="text-emerald-500">OS</span></h1>
                    <div className="flex items-center gap-1.5 mt-1 overflow-hidden">
                        <span className="flex items-center gap-1 text-[8px] md:text-[10px] font-black bg-emerald-100 text-emerald-700 px-1.5 md:px-2 py-0.5 rounded-full whitespace-nowrap">
                            <Flame className="w-2.5 h-2.5 md:w-3 md:h-3" /> {count} ACTIVE
                        </span>
            ...
            <div className="flex items-center gap-2 md:gap-4 shrink-0">
                <div className="text-right hidden sm:block">
                    <p className="text-lg md:text-3xl font-black text-slate-900 tabular-nums leading-none">{timeInfo.time}</p>
                </div>
...
export default function KitchenPage() {
  ...
  return (
    <div className="flex h-[100dvh] w-full bg-[#F1F5F9] font-sans text-slate-900 overflow-hidden flex-col relative">
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden md:overflow-y-hidden md:overflow-x-auto p-4 md:p-6 pb-32 scroll-smooth custom-scrollbar">
          <div className="flex flex-col md:flex-row gap-6 md:gap-6 h-auto md:h-full min-w-full md:min-w-[1200px] justify-center">
              ...
              <div className="flex-1 min-h-[40vh] md:min-h-0 md:h-full">
                <TicketColumn 
                    title="PENDING ORDERS" count={pendingTickets.length} tickets={pendingTickets} status="pending"
      ...
      {/* --- PREMIUM DETAIL MODAL (Fully Responsive & Non-Overflowing) --- */}
      <AnimatePresence>
          {selectedTicket && (
                          ...
                          {selectedTicket.order_items.map((item) => {
                              ...
                              return (
                                  <div 
                                      ...
                                      className={`p-4 md:p-5 rounded-2xl border flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 transition-all shadow-sm ${
                                  ...
                                  >
                                              ...
                                              <div className="flex flex-wrap gap-1.5 mt-1">
                                                  ...
                                                  {item.notes && (
                                                      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-red-50 text-red-600 border border-red-100">
                                                          ...
                                                          <span className="text-[9px] md:text-[10px] font-black uppercase tracking-wide truncate max-w-[150px]">{item.notes}</span>
                                                      </span>
                                                  )}
                                              </div>
                                          </div>
                                      ...
                                      <div className="flex flex-wrap gap-2 items-center sm:items-end justify-end shrink-0 sm:ml-auto w-full sm:w-auto">
                                          <button 
                                              ...
                                              {safeStatus === 'served' ? <Check className="w-3.5 h-3.5 md:w-4 md:h-4"/> : safeStatus === 'ready' ? <CheckCheck className="w-3.5 h-3.5 md:w-4 md:h-4" /> : safeStatus === 'cooking' ? <Flame className="w-3.5 h-3.5 md:w-4 md:h-4" /> : <Clock className="w-3.5 h-3.5 md:w-4 md:h-4" />}
                      ...
                      <div className="p-4 md:p-6 bg-slate-50 border-t border-slate-200 flex flex-wrap sm:flex-nowrap gap-3 shrink-0 pb-safe">
                          {selectedTicket.status !== 'cooking' && selectedTicket.status !== 'ready' && (
                              <button onClick={() => handleMoveTicket(selectedTicket.id, 'cooking')} className="flex-1 w-full sm:w-auto py-3.5 md:py-5 rounded-xl md:rounded-2xl bg-orange-500 hover:bg-orange-600 text-white font-black text-xs md:text-sm shadow-xl shadow-orange-500/20 active:scale-95 transition-all flex items-center justify-center gap-2 uppercase tracking-widest transform-gpu">
                                  <Flame className="w-4 h-4 md:w-5 md:h-5" /> Start Preparing
                              </button>
                          )}
        ...
        <div className={`w-full flex-1 flex flex-col h-full md:rounded-[2.5rem] border-2 md:border-dashed ${color} overflow-hidden transition-all duration-300 relative bg-white/50 backdrop-blur-sm rounded-3xl transform-gpu`}>
            <div className="flex px-4 md:px-6 py-4 md:py-5 justify-between items-center sticky top-0 z-10 bg-white/60 backdrop-blur-md border-b border-slate-100">
                <h3 className="font-black text-slate-500 text-[10px] md:text-xs flex items-center gap-1.5 md:gap-2 uppercase tracking-wider">
                    {status === 'pending' ? <Bell className="w-3.5 h-3.5 md:w-4 md:h-4" /> : status === 'cooking' ? <Flame className="w-3.5 h-3.5 md:w-4 md:h-4" /> : <CheckCircle2 className="w-3.5 h-3.5 md:w-4 md:h-4" />}
                    {title}
                </h3>
            ...
            <div className="flex-1 overflow-y-auto p-3 md:p-4 space-y-3 md:space-y-4 custom-scrollbar">
                ...
                {tickets.length === 0 && (
                    <div className="h-48 md:h-64 flex flex-col items-center justify-center text-slate-300 opacity-60">
                        <div className="w-12 h-12 md:w-16 md:h-16 bg-slate-100 rounded-full flex items-center justify-center mb-3">
                            <Utensils className="w-5 h-5 md:w-6 md:h-6" />
                        </div>
                        <span className="text-[9px] md:text-[10px] font-bold uppercase tracking-widest">No Orders</span>
                    </div>
                )}
            </div>
        </div>
...
function SwipeableTicket({ ticket, status, onSwipe, onOpen }: any) {
    ...
    return (
        <motion.div 
            ...
            className="cursor-pointer relative touch-pan-y transform-gpu"
        >
            {status !== 'ready' && (
    ...
    return (
        <div className={`w-full ${isLate ? 'ring-2 ring-red-100 rounded-2xl md:rounded-[1.8rem]' : ''}`}>
                            ...
                            <div className="flex justify-between items-start text-xs md:text-sm w-full">
                                ...
                                <div className="flex items-center shrink-0 ml-1 mt-0.5">
                                    {isDone ? (
                                        ...
                                        <span className="text-[8px] md:text-[9px] font-black uppercase tracking-wider text-orange-600 bg-orange-50 px-1.5 py-0.5 rounded border border-orange-100 flex items-center gap-0.5 animate-pulse"><Flame className="w-2 h-2 md:w-2.5 md:h-2.5"/> Cook</span>
                                    ) : (
                                        <span className="text-[8px] md:text-[9px] font-black uppercase tracking-wider text-slate-500 bg-slate-100 px-1.5 py-0.5 rounded border border-slate-200 flex items-center gap-0.5"><Clock className="w-2 h-2 md:w-2.5 md:h-2.5"/> Wait</span>
                                    )}
                                </div>
                            </div>
                        </div>
            ...
            <div className="pt-3 md:pt-4 border-t border-slate-100 flex items-center justify-between">
                <div className="w-16 md:w-20 h-1 md:h-1.5 bg-slate-100 rounded-full overflow-hidden">
