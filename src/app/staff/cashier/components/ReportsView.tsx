...
export default function ReportsView({ data }: any) {
    ...
    const filteredBills = useMemo(() => {
        return report?.bills?.filter((b: any) => {
            ...
            const cName = b.customer_name || "";
            const tNo = b.table_no || "";
            const search = searchTerm.toLowerCase();
            return displayBill.toLowerCase().includes(search) ||
                   tNo.toLowerCase().includes(search) ||
    ...
    return (
        <div className="p-4 md:p-8 h-full overflow-y-auto bg-[#F8FAFC] pb-[140px] custom-scrollbar transform-gpu">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-8">
                ...
                {/* DATE RANGE SELECTOR */}
                <div className="flex gap-2 bg-white p-1.5 rounded-2xl shadow-sm border border-slate-200 overflow-x-auto w-full md:w-auto no-scrollbar shrink-0">
                    {[1, 7, 30, 90].map(d => (
                        <button 
                            key={d} 
                            onClick={() => setRange(d)} 
                            className={`px-5 py-2.5 rounded-xl text-xs font-black transition-all whitespace-nowrap active:scale-95 ${range === d ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/30' : 'text-slate-500 hover:bg-emerald-50 hover:text-emerald-700'}`}
                        >
                            {d === 1 ? 'Today' : `${d} Days`}
                        </button>
                    ))}
            ...
            {loading ? (
                ...
                <motion.div variants={containerVariants} initial="hidden" animate="show" className="space-y-6 md:space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
                        <motion.div variants={itemVariants} className="bg-gradient-to-br from-emerald-600 to-teal-800 p-6 md:p-8 rounded-[2rem] md:rounded-[2.5rem] text-white shadow-2xl shadow-emerald-600/20 relative overflow-hidden group">
                            ...
                            <div className="relative z-10">
                                ...
                                <h2 className="text-4xl md:text-5xl font-black tracking-tighter truncate">{formatRs(report.summary.total)}</h2>
                    ...
                    <motion.div variants={itemVariants} className="bg-white rounded-[2rem] md:rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden transform-gpu">
                        ...
                        {/* Table Controls */}
                        <div className="p-4 md:p-6 border-b border-slate-100 flex flex-col md:flex-row justify-between items-center gap-3 bg-slate-50/50">
                            <div className="flex items-center gap-3 bg-white px-4 py-3 rounded-2xl border border-slate-200 w-full md:max-w-md shadow-sm focus-within:ring-2 focus-within:ring-emerald-100 transition-all">
                                <Search className="w-5 h-5 text-slate-400 shrink-0" />
                                <input placeholder="Search bill, table, or customer..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="bg-transparent outline-none font-bold text-sm w-full text-slate-700 placeholder:text-slate-400" />
                                {searchTerm && <button onClick={() => setSearchTerm("")}><X className="w-4 h-4 text-slate-400 hover:text-slate-600" /></button>}
                            </div>
                            <button onClick={() => exportToCSV(report.bills, range)} className="w-full md:w-auto px-6 py-3.5 bg-slate-900 text-white rounded-2xl text-xs font-black uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-emerald-600 hover:shadow-lg hover:shadow-emerald-500/30 transition-all active:scale-95">
