...
import Sidebar from "@/app/staff/manager/Sidebar"; // ROUTED TO MANAGER SIDEBAR
...
export default function ManagerInventoryPage() {
  ...
  // Financial Modal States (PREMIUM COMBOBOX)
  ...
  useEffect(() => { 
      loadData(false); // Initial visible load with spinner
      
      // Silently fetch fresh data every 5 seconds to keep Inventory synced with POS perfectly!
  ...
  useEffect(() => {
      setIsAdvancedUnit(selectedCategory === 'alcohol' || selectedCategory === 'drinks');
  }, [selectedCategory]);

  useEffect(() => {
      setFinanceCategory(financeType === 'expense' ? "Supplier Purchase" : "Manual POS Entry");
      setIsFinCatOpen(false);
  }, [financeType]);

  // Modified loadData accepts a 'silent' parameter so it doesn't show loading spinners during background syncs
  ...
  async function handleSaveFinancial(e: React.FormEvent) {
      ...
      if(res.success) {
          ...
          toast.success(`${financeType === 'income' ? 'Income' : 'Expense'} Logged!`, { description: `Successfully recorded under "${financeCategory}".` });
      } else toast.error(res.error || `Failed to log record.`);
      setIsSubmitting(false);
  }

  const triggerDelete = (id: string, type: 'inventory' | 'expense', title: string) => {
  ...
  async function confirmDelete() {
      ...
      const res = deleteTarget.type === 'inventory' 
          ? await deleteInventoryItem(deleteTarget.id) 
          : await deleteExpense(deleteTarget.id);
          
      if (res.success) {
          toast.success("Deleted Successfully", { description: `${deleteTarget.title.replace(/\[INC\]|\[EXP\]/g, '')} has been removed.` });
      } else {
  ...
  return (
    <div className="flex h-screen bg-[#F8FAFC] font-sans text-slate-900 relative md:flex-row flex-col overflow-hidden">
      <Sidebar tenantName={tenant?.name} tenantCode={tenant?.code} logo={tenant?.logo_url} />
      
      {/* --- PREMIUM DELETE MODAL (PERFECT MOBILE CENTERING) --- */}
      <AnimatePresence>
          {deleteTarget && (
              <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
                  ...
                  <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative w-full max-w-sm bg-white rounded-[2.5rem] shadow-2xl p-8 text-center border border-slate-100 overflow-hidden z-10">
                      ...
                      <p className="text-sm font-medium text-slate-500 mb-8 leading-relaxed">
                          This will permanently delete <br/><span className="font-bold text-slate-800">"{deleteTarget.title.replace(/\[INC\]|\[EXP\]/gi, '')}"</span>. This action cannot be undone.
                      </p>
      ...
      <main className="flex-1 p-4 md:p-8 overflow-y-auto relative w-full custom-scrollbar">
        ...
        {!hasAccess && (
            <div className="absolute inset-0 z-50 flex items-center justify-center backdrop-blur-md bg-white/40 p-4">
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white p-8 md:p-10 rounded-[2rem] shadow-2xl border border-slate-100 w-full max-w-md text-center relative overflow-hidden">
                    ...
                    <p className="text-slate-500 font-medium mb-8 leading-relaxed text-sm md:text-base">Inventory, Peg Tracking, and Expense Management are available on the Business Plan.</p>
                </motion.div>
            </div>
        )}
        ...
        <div className={`${!hasAccess ? 'blur-sm pointer-events-none opacity-50' : ''} transition-all duration-500`}>
            ...
            {/* PREMIUM HEADER - MOVED SEARCH DOWN */}
            <header className="flex flex-col gap-6 mb-8">
                ...
                <div className="flex flex-col lg:flex-row gap-3 w-full mt-2">
                    <div className="relative flex-1 w-full">
                        ...
                        <input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder={`Search ${activeTab}...`} className="w-full h-14 pl-14 pr-5 bg-white border-2 border-slate-100 rounded-2xl font-bold text-slate-700 outline-none focus:border-slate-900 focus:ring-4 focus:ring-slate-900/5 transition-all shadow-sm text-base" />
                    </div>
            ...
            <div className="flex gap-6 mb-8 border-b-2 border-slate-100">
                {['inventory', 'expenses'].map((tab) => (
                    <button key={tab} onClick={() => setActiveTab(tab as any)} className={`pb-4 px-2 font-black text-xs uppercase tracking-widest transition-all relative ${activeTab === tab ? 'text-slate-900' : 'text-slate-400 hover:text-slate-600'}`}>
                        ...
                        {activeTab === tab && <motion.div layoutId="activetabManager" className="absolute bottom-[-2px] left-0 w-full h-1 rounded-t-full bg-slate-900" />}
                    </button>
                ))}
            </div>
                    ...
                    <table className="w-full text-left min-w-[800px]">
                        ...
                        <tbody className="divide-y divide-slate-50">
                            {activeTab === 'inventory' ? filteredItems.map((item) => (
                                ...
                                // SECURELY IDENTIFY INCOME VS EXPENSE VIA DB TAG
                                ...
                                // CLEAN THE STRING SO THE TAG DOESN'T SHOW TO THE USER
        ...
        {/* --- ADD FINANCIAL LOG MODAL (PERFECT MOBILE CENTERING) --- */}
        <AnimatePresence>
            {isExpenseModalOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    ...
                    <motion.div initial={{ opacity: 0, scale: 0.95, y: 30 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 30 }} className="relative w-full max-w-lg max-h-[90vh] bg-white rounded-[2.5rem] shadow-2xl p-6 md:p-8 border border-slate-100 overflow-y-auto custom-scrollbar z-10">
                        
                        <div className="flex justify-between items-center mb-6">
                            <h2 className="text-2xl font-black text-slate-900">Log Cashflow</h2>
                            <button type="button" onClick={() => setIsExpenseModalOpen(false)} className="p-2 bg-slate-50 hover:bg-slate-100 rounded-full transition-colors"><X className="w-5 h-5 text-slate-500" /></button>
                        </div>
                        ...
                        <div className="flex p-1.5 bg-slate-100/80 border border-slate-200/50 rounded-[1.25rem] mb-6 relative">
                            <motion.div 
                                className={`absolute inset-y-1.5 w-[calc(50%-6px)] rounded-xl shadow-sm ${financeType === 'expense' ? 'bg-white left-1.5' : 'bg-white right-1.5'}`} 
                        ...
                        <form onSubmit={handleSaveFinancial} className="space-y-5">
                            ...
                            <div className="relative" ref={finCatDropdownRef}>
                                <label className="text-[10px] font-black uppercase text-slate-400 ml-1 mb-1 flex items-center gap-1">
                                    <Edit3 className="w-3 h-3" /> {financeType === 'income' ? 'Income Title / Category' : 'Expense Title / Category'}
                                </label>
                                <div className={`flex items-center w-full h-14 px-4 bg-slate-50 border-2 rounded-2xl transition-all focus-within:bg-white ${financeType === 'income' ? 'border-emerald-100 focus-within:border-emerald-400 focus-within:ring-4 focus-within:ring-emerald-500/10' : 'border-orange-100 focus-within:border-orange-400 focus-within:ring-4 focus-within:ring-orange-500/10'}`}>
                                    <input 
                                        ...
                                        onChange={(e) => { setFinanceCategory(e.target.value); setIsFinCatOpen(true); }}
                                        onFocus={() => setIsFinCatOpen(true)}
                                        placeholder={financeType === 'income' ? "e.g. Wedding Party Catering" : "e.g. Plumber Fixing Sink"}
                                        className={`w-full h-full bg-transparent outline-none font-bold text-base placeholder:font-medium placeholder:opacity-50 ${financeType === 'income' ? 'text-emerald-900' : 'text-orange-900'}`}
                                        autoComplete="off"
                                    />
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="text-[10px] font-black uppercase text-slate-400 ml-1 mb-1 block">Amount (Rs)</label>
                                    <div className={`flex items-center w-full h-14 px-4 border-2 rounded-2xl transition-all focus-within:bg-white bg-slate-50 ${financeType === 'income' ? 'border-emerald-100 focus-within:border-emerald-400 focus-within:ring-4 focus-within:ring-emerald-500/10 text-emerald-700' : 'border-orange-100 focus-within:border-orange-400 focus-within:ring-4 focus-within:ring-orange-500/10 text-orange-700'}`}>
                                        <span className="font-black opacity-50 mr-1">Rs</span>
                            ...
                            <div>
                                <label className="text-[10px] font-black uppercase text-slate-400 ml-1 mb-1 block">Detailed Description (Optional)</label>
                                <textarea name="description" placeholder="Add invoice numbers, details, or reasons here..." className="w-full h-24 p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl font-medium text-slate-900 outline-none focus:bg-white focus:border-slate-900 focus:ring-4 focus:ring-slate-900/10 transition-all resize-none placeholder:text-slate-400" />
                            </div>
                            ...
                            <div className="pt-2 flex gap-3">
                                ...
                                <button type="submit" disabled={isSubmitting} className={`flex-1 h-14 rounded-2xl text-white font-black text-lg transition-all active:scale-95 flex items-center justify-center gap-2 ${financeType === 'income' ? 'bg-emerald-600 hover:bg-emerald-700 shadow-xl shadow-emerald-500/20' : 'bg-slate-900 hover:bg-slate-800 shadow-xl shadow-slate-900/20'}`}>
                                    {isSubmitting ? <Loader2 className="animate-spin" /> : <>Save {financeType}</>}
                                </button>
                            </div>
                        </form>
                    </motion.div>
        ...
        {/* --- ADD STOCK MODAL (PERFECT MOBILE CENTERING) --- */}
        <AnimatePresence>
            {isModalOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    ...
                    <motion.div initial={{ opacity: 0, scale: 0.95, y: 30 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 30 }} className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto bg-white rounded-[2.5rem] shadow-2xl p-6 md:p-8 border border-slate-100 custom-scrollbar z-10">
                        ...
                        <form onSubmit={handleSaveItem} className="space-y-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div className="relative" ref={catDropdownRef}>
                                    ...
                                    <div onClick={() => setIsCatOpen(!isCatOpen)} className="w-full h-14 px-5 bg-slate-50 border-2 border-slate-100 rounded-2xl font-bold flex items-center justify-between cursor-pointer focus-within:bg-white focus-within:border-slate-900 transition-all">
                                        <span className="capitalize">{selectedCategory}</span><ChevronDown className="w-5 h-5 text-slate-400" />
                                    </div>
                                    <AnimatePresence>{isCatOpen && (<motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="absolute z-[120] top-[75px] left-0 w-full bg-white border-2 border-slate-100 rounded-2xl shadow-2xl p-2"><div className="max-h-40 overflow-y-auto mb-2 space-y-1 custom-scrollbar">{allCategories.map(cat => (<div key={cat} onClick={() => { setSelectedCategory(cat); setIsCatOpen(false); }} className="p-3 rounded-xl hover:bg-slate-50 cursor-pointer font-bold text-sm capitalize">{cat}</div>))}</div><div className="border-t border-slate-100 pt-2"><input value={customCategory} onChange={(e) => setCustomCategory(e.target.value)} placeholder="Type new & press Enter..." className="w-full text-sm font-medium px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg outline-none focus:bg-white focus:border-emerald-500 transition-all" onKeyDown={(e) => { if (e.key === 'Enter' && customCategory.trim()) { e.preventDefault(); setSelectedCategory(customCategory.toLowerCase().trim()); setCustomCategory(""); setIsCatOpen(false); } }} /></div></motion.div>)}</AnimatePresence>
                                </div>
                            </div>
                            ...
                            <div className="relative" ref={menuDropdownRef}>
                                ...
                                <div onClick={() => setIsMenuLinkOpen(!isMenuLinkOpen)} className="w-full h-14 px-5 bg-emerald-50/50 border-2 border-emerald-100 rounded-2xl font-bold text-emerald-800 flex items-center justify-between cursor-pointer focus-within:ring-4 focus-within:ring-emerald-500/20 transition-all">
                                    <span className="truncate">{selectedMenuLink || "Manual (Not Linked)"}</span><ChevronDown className="w-5 h-5 text-emerald-600" />
                                </div>
                                <AnimatePresence>{isMenuLinkOpen && (
                                    <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="absolute z-[120] top-[75px] left-0 w-full bg-white border-2 border-slate-100 rounded-2xl shadow-2xl p-3">
                                        <input autoFocus value={menuSearch} onChange={e => setMenuSearch(e.target.value)} placeholder="Search menu items..." className="w-full h-11 px-4 bg-slate-50 border border-slate-100 rounded-xl mb-3 text-sm font-bold outline-none focus:border-emerald-400" />
                                        <div className="max-h-48 overflow-y-auto space-y-1 custom-scrollbar">
                                            <div onClick={() => { setSelectedMenuLink(""); setIsMenuLinkOpen(false); }} className="p-3 rounded-xl hover:bg-slate-50 cursor-pointer font-bold text-sm text-slate-400 italic">Unlink / Manual Tracking</div>
                                            {posMenuItems.filter(m => m.toLowerCase().includes(menuSearch.toLowerCase())).map(m => (<div key={m} onClick={() => { setSelectedMenuLink(m); setIsMenuLinkOpen(false); }} className="p-3 rounded-xl hover:bg-emerald-50 text-emerald-900 cursor-pointer font-bold text-sm">{m}</div>))}
                                        </div>
                                    </motion.div>
                                )}</AnimatePresence>
                            </div>
                            ...
                            <div className="flex items-center justify-between p-5 bg-slate-50 border border-slate-200 rounded-2xl">
                                <div><span className="font-black text-slate-900 text-sm flex items-center gap-2"><Layers className="w-4 h-4 text-slate-400"/> Track by Sub-units?</span><p className="text-[10px] font-bold text-slate-400 mt-1">Example: 1 Case = 24 Cans, 1 Bottle = 750ml</p></div>
