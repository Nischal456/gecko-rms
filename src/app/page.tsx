...
function LaunchCountdownCard({ isLoaded }: { isLoaded: boolean }) {
    ...
    return (
        <>
                    ...
                    <div className="relative z-10 flex flex-col items-center text-center font-sans">
                        ...
                        <div className="flex items-center gap-3 mb-6 bg-slate-50/80 px-5 py-2 rounded-2xl border border-slate-200/60 shadow-sm">
                            ...
                            <span className="text-[10px] sm:text-xs font-black text-emerald-600 uppercase tracking-widest flex items-center gap-1.5">
                                ...
                                {timeLeft.isExpired ? "OFFICIALLY LIVE" : "GRAND LAUNCH TOMORROW"}
                            </span>
                        </div>
                        ...
                        <p className="text-xs sm:text-sm font-black text-slate-400 uppercase tracking-widest mb-5">
                            {timeLeft.isExpired ? "WELCOME TO THE FUTURE OF DINING" : "UNTIL TOMORROW'S GRAND REVEAL"}
                        </p>
                        ...
                        <div className="flex flex-wrap items-center justify-center gap-2.5 sm:gap-4 pt-4 border-t border-slate-100 w-full text-xs font-bold text-slate-600 font-sans">
                            <span className="bg-emerald-100/80 text-emerald-800 px-3.5 py-1.5 rounded-full border border-emerald-200 flex items-center gap-1.5 font-bold font-sans">
                                🇳🇵 Bhadra 1, 2083 (Tomorrow)
                            </span>
                            <span className="bg-slate-100 text-slate-700 px-3.5 py-1.5 rounded-full border border-slate-200 font-bold font-sans">
            ...
            {/* --- GRAND LAUNCH CELEBRATION & BOOKING POPUP MODAL --- */}
            <AnimatePresence>
                {isCelebrationModalOpen && (
                    <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md">
                        <motion.div
                            ...
                            className="bg-white rounded-[2.5rem] p-6 sm:p-10 max-w-lg w-full shadow-2xl border-2 border-emerald-500/80 relative overflow-hidden text-center font-sans"
                        >
                            {/* Glowing Background Glow & Popper Effect */}
                            <div className="absolute top-0 inset-x-0 h-36 bg-gradient-to-b from-emerald-500/15 via-teal-500/5 to-transparent pointer-events-none" />
                            ...
                            <button
                                ...
                                className="absolute top-4 right-4 w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 hover:bg-slate-200 transition-colors z-20"
                            >
                                <X className="w-4 h-4" />
                            </button>

                            <div className="relative z-10 flex flex-col items-center">
                                <div className="w-16 h-16 bg-emerald-100 rounded-full flex items-center justify-center mb-4 shadow-md border border-emerald-200 animate-bounce">
                                    <Rocket className="w-8 h-8 text-emerald-600" />
                                </div>

                                <span className="bg-emerald-50 border border-emerald-200 text-emerald-700 text-[10px] font-black uppercase tracking-widest px-4 py-1.5 rounded-full mb-3 shadow-sm font-sans">
                                    🚀 GRAND LAUNCH • BHADRA 1, 2083
                                </span>

                                <h3 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight mb-2">
                                    Gecko RMS is Officially Live!
                                </h3>

                                <p className="text-xs sm:text-sm text-slate-500 font-medium mb-6 leading-relaxed">
                                    Nepal's fastest zero-lag restaurant operating system is now open worldwide. Claim your 15-day free trial today!
                                </p>

                                {/* Pricing Highlight Box inside Celebration Modal */}
                                <div className="w-full bg-slate-50 rounded-2xl p-4 border border-slate-200/80 mb-6 text-left space-y-2">
                                    <div className="flex justify-between items-center text-xs font-bold">
                                        <span className="text-slate-500">Subscription Plan:</span>
                                        <span className="text-slate-900 font-black">Gecko RMS Pro (Rs 1,499/mo)</span>
                                    </div>
                                    <div className="flex justify-between items-center text-xs font-bold">
                                        <span className="text-slate-500">Free Trial:</span>
                                        <span className="text-emerald-600 font-black">15 Days Risk-Free</span>
                                    </div>
                                    <div className="flex justify-between items-center text-xs font-bold">
                                        <span className="text-slate-500">Annual Offer:</span>
                                        <span className="text-emerald-600 font-black">3 Months Extra FREE + Rs 0 Deposit</span>
                                    </div>
                                </div>

                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full">
                                    <Link href="/pricing" className="w-full" onClick={() => setIsCelebrationModalOpen(false)}>
                                        <button className="w-full py-3.5 bg-emerald-600 hover:bg-emerald-700 text-white font-black text-xs rounded-xl shadow-lg shadow-emerald-600/30 transition-all flex items-center justify-center gap-2">
                                            Book Now & Pricing
                                            <ArrowRight className="w-4 h-4" />
                                        </button>
                                    </Link>
                                    <Link href="/signup" className="w-full" onClick={() => setIsCelebrationModalOpen(false)}>
                                        <button className="w-full py-3.5 bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs rounded-xl transition-all flex items-center justify-center gap-2">
                                            Start 15-Day Trial
                                        </button>
                                    </Link>
                                </div>
